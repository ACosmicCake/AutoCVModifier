...
from .pdf_generator import generate_cv_pdf_from_json_string, create_cover_letter_pdf # Returns True/False
...
def create_app(test_config=None):
    ...
    def tailor_cv_endpoint():
        ...
        if file and allowed_file(file.filename):
            ...
            job_title_from_cv = cv_data.get("CV", {}).get("JobTitle", "Application")
            safe_job_title = secure_filename(job_title_from_cv) if job_title_from_cv else "Application"
    ...
    # --- PDF Generation for Cover Letter ---
    try:
        cv_data = json.loads(cv_json_str)
        applicant_name = cv_data.get("CV", {}).get("PersonalInformation", {}).get("Name", "UnknownApplicant")
        safe_applicant_name = secure_filename(applicant_name) if applicant_name else "UnknownApplicant"
        job_title = cv_data.get("CV", {}).get("JobTitle", "Application")
        safe_job_title = secure_filename(job_title) if job_title else "Application"
    except (json.JSONDecodeError, AttributeError):
        safe_applicant_name = "UnknownApplicant"
        safe_job_title = "Application"

    base_pdf_filename = f"CoverLetter_{safe_job_title}_{safe_applicant_name}.pdf"
    pdf_folder = app.config['GENERATED_PDFS_FOLDER']

    final_pdf_filename = base_pdf_filename
    counter = 1
    while os.path.exists(os.path.join(pdf_folder, final_pdf_filename)):
        final_pdf_filename = f"CoverLetter_{safe_job_title}_{safe_applicant_name}_{counter}.pdf"
        counter += 1

    full_pdf_path = os.path.join(pdf_folder, final_pdf_filename)

    pdf_generation_success = create_cover_letter_pdf(cover_letter_text, full_pdf_path)

    if pdf_generation_success:
        return jsonify({
            "message": "Cover Letter Generated and PDF Created!",
            "cover_letter_text": cover_letter_text,
            "pdf_download_url": f"/api/download-cv/{final_pdf_filename}"
        })
    else:
        return jsonify({
            "message": "Cover Letter Generated, but PDF creation failed.",
            "cover_letter_text": cover_letter_text
        }), 500
